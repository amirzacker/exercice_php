    ...
    if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
        ...
        if($_POST['password'] == $_POST['confirm_password']){
           ...
           if(strlen($_POST['password']) >= 8 && strlen($_POST['email']) >= 8){
                ...
                $sqlSelectEmail = "SELECT * FROM users WHERE email ='".$email_inscription."'";
                ...
                if(mysqli_num_rows($emailResultat) > 0){
                    ...
                    $sql = "INSERT INTO users (id_user, lastname, firstname, email, password, admin) VALUES (NULL,'".$_POST["firstname"]."','".$_POST["lastname"]."','".$_POST["email"]."','".$cryptage_sha."',false)";
